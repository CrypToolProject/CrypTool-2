...
namespace SharpPcap.LibPcap
{
    ...
    public class PcapStatistics : ICaptureStatistics
    {
        ...
        internal PcapStatistics(IntPtr pcap_t)
        {
            ...
            // retrieve the stats
            PcapUnmanagedStructures.PcapStatReturnValue result = (PcapUnmanagedStructures.PcapStatReturnValue)LibPcapSafeNativeMethods.pcap_stats(pcap_t, stat);
            ...
            switch (result)
            {
                case PcapUnmanagedStructures.PcapStatReturnValue.Error:
                    ...
                    string error = LibPcapLiveDevice.GetLastError(pcap_t);
            ...
            if (Environment.OSVersion.Platform == PlatformID.Unix)
            {
                PcapUnmanagedStructures.pcap_stat_unix managedStat = (PcapUnmanagedStructures.pcap_stat_unix)Marshal.PtrToStructure(stat,
                                                                                                 typeof(PcapUnmanagedStructures.pcap_stat_unix));

                // copy the values
                ReceivedPackets = (uint)managedStat.ps_recv;
                DroppedPackets = (uint)managedStat.ps_drop;
                //                this.InterfaceDroppedPackets = (uint)managedStat.ps_ifdrop;
            }
            else
            {
                PcapUnmanagedStructures.pcap_stat_windows managedStat = (PcapUnmanagedStructures.pcap_stat_windows)Marshal.PtrToStructure(stat,
