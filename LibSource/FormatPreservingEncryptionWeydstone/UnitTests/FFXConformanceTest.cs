...
namespace FPETests
{
     ...
     */
    [TestClass]
    public class FFXConformanceTest
    {
        ...
        */
        ...
        public void testFF1Conformance()
        {
            ...
            foreach (TestInput test in ff1Tests)
            {
                ...
                // create an FF1 instance of FFX
                FFX ffx = new FFX(new FF1Parameters(test.radix));
                ...
                int[] CT = ffx.encrypt(test.key, test.tweak, test.plaintext);
                ...
                int[] PT = ffx.decrypt(test.key, test.tweak, CT);
        ...
        public void testFF3Conformance()
        {
            ...
            foreach (TestInput test in ff3Tests)
            {
                ...
                try
                {
                    // create an FF3 instance of FFX
                    FFX ffx = new FFX(new FF3Parameters(test.radix));
                    Assert.IsNotNull(ffx);
                    ...
                    // perform the decryption
                    int[] PT = ffx.decrypt(test.key, test.tweak, test.ciphertext);
