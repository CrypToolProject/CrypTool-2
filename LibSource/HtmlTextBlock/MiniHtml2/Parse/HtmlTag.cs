...
namespace QuickZip.MiniHtml2
{
    ...
    public class HtmlTag
    {
        ...
        internal int ID => Math.Abs(Utils.LocateTag(Utils.RemoveFrontSlash(name)));
        ...
        internal int Level
        {
            get
            {
                ...
                {
                    return Defines.BuiltinTags[ID].tagLevel;
                }
            }
        }

        internal bool IsEndTag => ((name.IndexOf('/') == 0) || (variables.Contains('/')));

        ///<summary> Gets HtmlTag name. (without <>) </summary>
        public string Name => name;
        ///<summary> Gets variable value. </summary>
        public string this[string key] => variables[key].value;
