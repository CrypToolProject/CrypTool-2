...
static void dump_lanczos_state(msieve_obj *obj, 
	...
	if (status == 0) {
        ...
        throwException("error: cannot write new checkpoint file\nerror: previous checkpoint file not overwritten\n");
	}
#if 1
	...
	if (rename(buf, buf_old)) {
        ...
        throwException("error: cannot update checkpoint file\n");
	}
...
uint64 * block_lanczos(msieve_obj *obj, 
	...
	if (max_ncols <= max_nrows) {
        ...
        throwException("matrix needs more columns than rows; "
            "try adding 2-3%% more relations\n");
	}
