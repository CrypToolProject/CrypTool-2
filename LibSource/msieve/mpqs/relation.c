...
void save_relation(sieve_conf_t *conf, uint32 sieve_offset,
	...
	struct relation* rel;
	const int len = 32 * MAX_MP_WORDS / 4;
	...
	if (conf->slave)
	{
		...
		conf->relationPackage->package_array[conf->relationPackage->package_count - 1].type = 0;
		rel = &conf->relationPackage->package_array[conf->relationPackage->package_count - 1].rel;
		
		fb_offsets_copy = (uint32*)malloc(len*sizeof(uint32));
		for (i = 0; i < len; i++)
			fb_offsets_copy[i] = fb_offsets[i];

		rel->fb_offsets = fb_offsets_copy;
		rel->large_prime1 = large_prime1;
		rel->large_prime2 = large_prime2;
		rel->num_factors = num_factors;
		rel->poly_index = poly_index;
		rel->sieve_offset = sieve_offset;
