...
#ifdef _M_CEE
//Workaround for usage in CT2: When compiling the C++/CLI Wrapper, pretend here that NLT_THREADS are not used to avoid
//compatibility problems between C++/CLI and std::mutex.
//Due to the fact that all *.cpp files of ExtendedNTL are compiled in separate native project "ExtendedNTLLib",
//NTL_THREADS will still be used by all code called from there.
#undef NTL_THREADS
#endif
