...
//
// $HeadURL: https://svn.CrypTool.org/CrypTool2/trunk/LibSource/SSCext/SSCwhirl.cs $
//////////////////////////////////////////////////////////////////////////////////////////////////
// $Revision:: 8109                                                                           $://
// $Author:: kopal                                                                            $://
...
namespace System.Security.Cryptography
{
    public class HMACWHIRLPOOL : HMAC
    {
        ...
        private const int DIGESTBITS = DIGESTBYTES * 8;
        private const int WBLOCKBYTES = 64;
        private const int WBLOCKBITS = WBLOCKBYTES * 8;
        private const int LENGTHBYTES = 32;
        private const int LENGTHBITS = LENGTHBYTES * 8;

        // The number of rounds of the internal dedicated block cipher.
        private const int R = 10;
        private byte[] bitLength;  // global number of hashed bits (256-bit counter) 
        private byte[] buffer;        // buffer of data to hash 
        private ulong bufferBits; // current number of bits on the buffer 
        private ulong bufferPos;  // current (possibly incomplete) byte slot on the buffer 
        private ulong[] hash;       // the hashing state
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            ...
            ulong sourceBits = (ulong)(cbSize - ibStart) * 8;

            // index of leftmost array ulong containing data (1 to 8 bits). 
            ulong sourcePos = (ulong)ibStart;

            // space on array[sourcePos]. 
            ...
            while (sourceBits > 8)
            {
                // N.B. at least array[sourcePos] and array[sourcePos+1] contain data. 
                // take a byte from the array:
                ulong s0 = array[sourcePos];
                ulong s1 = array[sourcePos + 1];
            ...
            // now 0 <= sourceBits <= 8;
            // furthermore, all data (if any is left) is in array[sourcePos].
            if (sourceBits > 0)
            {
                b = (byte)((array[sourcePos] << sourceGap) & 0x00ff); /* bits are left-justified on b. */
