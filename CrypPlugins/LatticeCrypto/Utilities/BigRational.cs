...
namespace LatticeCrypto.Utilities
{
    ...
    public struct BigRational : IComparable, IComparable<BigRational>, IDeserializationCallback, IEquatable<BigRational>, ISerializable
    {
        ...
        {
            BigInteger quotient = BigInteger.DivRem(m_numerator, m_denominator, out BigInteger rem);

            BigInteger div = m_denominator / 2;
            if (!m_denominator.IsEven)
            {
                div++;
            }
        ...
        public static explicit operator decimal(BigRational value)
        {
            ...
            for (int scale = DecimalMaxScale; scale >= 0; scale--)
            {
                ...
                {
                    DecimalUInt32 dec = new DecimalUInt32 { dec = (decimal)denormalized };
