 ...
 */
int AES_set_encrypt_key(global unsigned char *userKey, const int bits, private AES_KEY *key, int add)
{
	u32 *rk;
   	int i = 0;
	u32 temp;
 ...
 */
int AES_set_decrypt_key(global unsigned char *userKey, const int bits, private AES_KEY *key, int add) 
{
    u32 *rk;
	int i, j, status;
	u32 temp;
	...
	status = AES_set_encrypt_key(userKey, bits, key, add);
	if (status < 0)
		return status;
 ...
 */
void AES_decrypt(constant unsigned char *in, private unsigned char *out, private AES_KEY *key) 
{
	const u32 *rk;
