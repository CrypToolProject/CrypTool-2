...
namespace CrypTool.Fialka
{
    ...
    public class FialkaCore
    {
        ...
        public string encrypt(string inputStr, LetterProcesseddelegate del)
        {
            ...
            this.internalState.punchCard = this.internalState.punchCard;
            // workaround end
            ...
            {
                int inputChar = this.keyboardToZ30(inputStr[i]);
                if (inputChar == -1)
                {
                    if (this.internalState.inputHandler == FialkaEnums.handleInvalidInput.KeepUnchanged)
                    {
                        output.Append(inputStr[i]);
                    }
                    else if (this.internalState.inputHandler == FialkaEnums.handleInvalidInput.KeepAndMark)
                    {
                ...
                else
                {
                    ...
                    char outputChar = this.printHeadFromZ30(encryptedInputChar);
                    output.Append(outputChar);
                    // after encryption check the mixed mode, call only after printHead usage !!!
                    if (this.internalState.txtOpMode == FialkaEnums.textOperationmMode.Mixed) mixedModeShiftCheck(inputChar, encryptedInputChar);
                }
                // callback 
                if (del != null)
                {
        ...
        private void mixedModeShiftCheck(int input, int output)
        {
            ...
            switch (this.internalState.opMode)
            {
                ...
                case FialkaEnums.operationMode.Encrypt:
                {
                    ...
                    {
                        this.internalState.printHeadShift = FialkaEnums.printHeadShift.LetterShift;
                    }
                    else if (input == FialkaConstants.NumerShiftContact)
                    {
                        this.internalState.printHeadShift = FialkaEnums.printHeadShift.NumberShift;
                    }   
                }
                    break;
        ...
        private int encrypt30(int input)
        {
            ...
            input = this.internalState.punchCard[input];
            ...
            input = this.internalState.punchCardInverse[input];
        ...
        {
            // - offsets
            input += this.internalState.rotorOffsets[index]; // rotor offset
            input += this.internalState.ringOffsets[index]; // ring
            input -= this.internalState.coreOffsets[index]; // core offset 
            ...
            input = this.handleRotorWiringWithCoreOrientation(inverse, this.internalState.coreOrientation[index], index, input);
            // + offsets
            input -= this.internalState.rotorOffsets[index]; // rotor offset
            input -= this.internalState.ringOffsets[index]; // ring
            input += this.internalState.coreOffsets[index]; // core offset 
        ...
        private int handleRotorWiringWithCoreOrientation(int inverseWiring, int coreOrientation, int index, int input)
        {
            ...
            if (permType == 1) // normal perm
            {
                ...
                {
                    input = this.internalState.rotorWiringsFlippedCores[this.internalState.coreOrders[index], input];
                }
                else
                {
                    input = this.internalState.rotorWirings[this.internalState.coreOrders[index], input];
                }
            }
            else if (permType == -1) // inverse perm
            {
                if (coreOrientation == FialkaConstants.coreSideReverse)
                {
                    input = this.internalState.inverseRotorWiringsFlippedCores[this.internalState.coreOrders[index], input];
                }
                else
                {
                    input = this.internalState.inverseRotorWirings[this.internalState.coreOrders[index], input];
                }
            }
            else
            {
        ...
        private void rotorStepping()
        {
            ...
            for (int i = 1; i < 10; i += 2)
            {
                ...
                int checkPosition = (this.internalState.rotorOffsets[i] + this.internalState.ringOffsets[i] + FialkaConstants.evenRotorsPinCheckPosition) % FialkaConstants.mod;
                // 
                this.internalState.rotorOffsets[i] = (this.internalState.rotorOffsets[i] + FialkaConstants.mod - 1) % FialkaConstants.mod;
                if (this.internalState.pinPositions[this.internalState.rotorOrders[i], checkPosition] == 1)
                {
            ...
            for (int i = 8; i >= 0; i -= 2)
            {
                ...
                // we do not need to handle coreOffset it does not influence the pin position
                int checkPosition = (this.internalState.rotorOffsets[i] + this.internalState.ringOffsets[i] + FialkaConstants.oddRotorsPinCheckPosition) % FialkaConstants.mod;
                // 
                this.internalState.rotorOffsets[i] = (this.internalState.rotorOffsets[i] + 1) % FialkaConstants.mod;
                if (this.internalState.pinPositions[this.internalState.rotorOrders[i], checkPosition] == 1)
                {
                    break; // the next rotors are disconnected, no rotation
                }
            ...
            this.internalState.notifyInternalStatChangede();
        }
        #endregion
        ...
        public int keyboardToZ30(char input)
        {
            ...
            if (this.internalState.numlockType == FialkaEnums.numLockType.NumLock10)
            {
            ...
            {   // numLock 30 modes - necessary to check different cases
                ...
                {
                    if (this.internalState.opMode == FialkaEnums.operationMode.Decrypt)
                    {
                ...
                else if (this.internalState.opMode == FialkaEnums.operationMode.Encrypt &&
                         FialkaConstants.spaceDisabledKey(this.internalState.printHead, this.internalState.countryLayout, input))
                {
                ...
                else
                {
                    ...
                    switch (this.internalState.model)
                    {
                        ...
                        case FialkaEnums.machineModel.M125:
                            {
                                ...
                                if (this.internalState.printHead == FialkaEnums.printHeadMapping.Cyrillic)
                                {
                                    encoded = FialkaConstants.mapCyrillicInput(c);
                                }
                                else
                                {
                                    encoded = FialkaConstants.mapLatinInput(this.internalState.countryLayout, c);
                                }
                            } // end case M-125
                            break;
                        case FialkaEnums.machineModel.M125_3:
                            {
                                if (this.internalState.txtOpMode == FialkaEnums.textOperationmMode.Mixed)
                                {
                                ...
                                if (this.internalState.printHead == FialkaEnums.printHeadMapping.Cyrillic)
                                {
                                    if (this.internalState.printHeadShift == FialkaEnums.printHeadShift.LetterShift)
                                    {
                                        encoded = FialkaConstants.mapCyrillicInput(c);
                                    }
                                    else
                                ...
                                else
                                {
                                    ...
                                    {
                                        encoded = FialkaConstants.mapSpecialInput(this.internalState.countryLayout, c);
                                    }
                                }
                            } // end case M-125-3
                            break;
        ...
        public char printHeadFromZ30(int input)
        {
            ...
            if (input == FialkaConstants.spaceContactInDecryptMode && this.internalState.opMode == FialkaEnums.operationMode.Decrypt)
            {
            ...
            switch (this.internalState.printHead)
            {
                case FialkaEnums.printHeadMapping.Cyrillic:
                    {
                        // cyrillic print head
                       output = FialkaConstants.getPrinterCharCyrillic(this.internalState.printHeadShift, input);
                    } break;
                case FialkaEnums.printHeadMapping.Latin:
                    {
                        // latin print head
                        output = FialkaConstants.getPrinterCharLatin(this.internalState.countryLayout, this.internalState.printHeadShift, input);
                    } break;
            };
            return output;
        }
