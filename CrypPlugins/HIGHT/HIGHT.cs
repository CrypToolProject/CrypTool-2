...
namespace CrypTool.Plugins.HIGHT
{
    ...
    [PluginInfo("CrypTool.Plugins.HIGHT.Properties.Resources", "PluginCaption", "PluginTooltip", "HIGHT/DetailedDescription/doc.xml", "HIGHT/images/icon.png")]
    [ComponentCategory(ComponentCategory.CiphersModernSymmetric)]
    public class HIGHT : ICrypComponent
    {
        #region Private Variables

        private readonly HIGHTSettings _settings = new HIGHTSettings();
        ...
        public void Execute()
        {
            ...
            HIGHTAlgorithm hight = new HIGHTAlgorithm(_inputKey);
          
            //Create reference to encrypt or decrypt method of HIGHT
            BlockCipher blockCipher = null;
            if (_settings.Action == CipherAction.Encrypt ||
                _settings.BlockMode == BlockMode.CFB ||
                _settings.BlockMode == BlockMode.OFB)
            {
                blockCipher = new BlockCipher(hight.EncryptBlock);
            }
            else
            {
                blockCipher = new BlockCipher(hight.DecryptBlock);
            }
