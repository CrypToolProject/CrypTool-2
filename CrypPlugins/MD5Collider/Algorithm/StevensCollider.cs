...
namespace CrypTool.Plugins.MD5Collider.Algorithm
{
    class StevensCollider : MD5ColliderBase
    {
        ...
        void find_collision(UInt32[] IV, UInt32[] msg1block0, UInt32[] msg1block1, UInt32[] msg2block0, UInt32[] msg2block1)
        {
            ...
            UInt32[] IHV = new[] { IV[0], IV[1], IV[2], IV[3] };
        ...
        void MD5_REVERSE_STEP(UInt32[] block, UInt32[] Q, int t, UInt32 AC, int RC)
        {
        ...
        void find_block0(UInt32[] block, UInt32[] IV)
        {
            ...
            UInt32[] q4mask = new UInt32[1 << 4];
            for (UInt32 k = 0; k < q4mask.Length; ++k)
                q4mask[k] = ((k << 2) ^ (k << 26)) & 0x38000004;

            UInt32[] q9q10mask = new UInt32[1 << 3];
            for (UInt32 k = 0; k < q9q10mask.Length; ++k)
            ...
            while (true)
            {
                ...
                UInt32 tt1 = FF(Q[Qoff + 1], Q[Qoff + 0], Q[Qoff - 1]) + Q[Qoff - 2] + 0xe8c7b756;
                UInt32 tt17 = GG(Q[Qoff + 16], Q[Qoff + 15], Q[Qoff + 14]) + Q[Qoff + 13] + 0xf61e2562;
                UInt32 tt18 = Q[Qoff + 14] + 0xc040b340 + block[6];
                UInt32 tt19 = Q[Qoff + 15] + 0x265e5a51 + block[11];
                UInt32 tt20 = Q[Qoff + 16] + 0xe9b6c7aa + block[0];
                UInt32 tt5 = RR(Q[Qoff + 6] - Q[Qoff + 5], 12) - FF(Q[Qoff + 5], Q[Qoff + 4], Q[Qoff + 3]) - 0x4787c62a;
                ...
                UInt32 tt21 = GG(Q[Qoff + 20], Q[Qoff + 19], Q[Qoff + 18]) + Q[Qoff + 17] + 0xd62f105d;
                ...
                while (counter2 < (1 << 4))
                {
                    ...
                    UInt32 tt22 = GG(Q[Qoff + 21], Q[Qoff + 20], Q[Qoff + 19]) + Q[Qoff + 18] + 0x02441453;
                    UInt32 tt23 = Q[Qoff + 19] + 0xd8a1e681 + block[15];
                    UInt32 tt24 = Q[Qoff + 20] + 0xe7d3fbc8 + block[4];

                    UInt32 tt9 = Q[Qoff + 6] + 0x8b44f7af;
                    UInt32 tt10 = Q[Qoff + 7] + 0xffff5bb1;
                    UInt32 tt8 = FF(Q[Qoff + 8], Q[Qoff + 7], Q[Qoff + 6]) + Q[Qoff + 5] + 0x698098d8;
                    UInt32 tt12 = RR(Q[Qoff + 13] - Q[Qoff + 12], 7) - 0x6b901122;
                    UInt32 tt13 = RR(Q[Qoff + 14] - Q[Qoff + 13], 12) - FF(Q[Qoff + 13], Q[Qoff + 12], Q[Qoff + 11]) - 0xfd987193;
                    ...
                    for (UInt32 counter3 = 0; counter3 < (1 << 3); )
                    {
                        if (IsStopped)
                            return;
                        ...
                        for (UInt32 counter4 = 0; counter4 < (1 << 16); ++counter4)
                        {
                            if (IsStopped)
                                return;
                            ...
                            for (int t = 0; t < 16; ++t)
                                block2[t] = block[t];
                            block2[4] += (UInt32)1 << 31;
                            block2[11] += 1 << 15;
                            block2[14] += (UInt32)1 << 31;
        ...
        void MD5_STEP_FF(ref UInt32 a, UInt32 b, UInt32 c, UInt32 d, UInt32 m, UInt32 ac, Int32 rc)
        {
            a += (d ^ (b & (c ^ d))) + m + ac;
            a = (a << rc | a >> (32 - rc)) + b;
        }

        void MD5_STEP_GG(ref UInt32 a, UInt32 b, UInt32 c, UInt32 d, UInt32 m, UInt32 ac, Int32 rc)
        {
            a += (c ^ (d & (b ^ c))) + m + ac;
            a = (a << rc | a >> (32 - rc)) + b;
        }

        void MD5_STEP_HH(ref UInt32 a, UInt32 b, UInt32 c, UInt32 d, UInt32 m, UInt32 ac, Int32 rc)
        {
            a += (b ^ c ^ d) + m + ac;
            a = (a << rc | a >> (32 - rc)) + b;
        }
        void MD5_STEP_II(ref UInt32 a, UInt32 b, UInt32 c, UInt32 d, UInt32 m, UInt32 ac, Int32 rc)
        {
            a += (c ^ (b | ~d)) + m + ac;
            a = (a << rc | a >> (32 - rc)) + b;
        }
        ...
        void find_block1(UInt32[] block, UInt32[] IV)
        {
            if (((IV[1] ^ IV[2]) & (1 << 31)) == 0 && ((IV[1] ^ IV[3]) & (1 << 31)) == 0
                && (IV[3] & (1 << 25)) == 0 && (IV[2] & (1 << 25)) == 0 && (IV[1] & (1 << 25)) == 0 && ((IV[2] ^ IV[1]) & 1) == 0
               )
            {
                UInt32[] IV2 = new UInt32[] { (UInt32)(IV[0] + (1 << 31)), (UInt32)(IV[1] + (1 << 31) + (1 << 25)), (UInt32)(IV[2] + (1 << 31) + (1 << 25)), (UInt32)(IV[3] + (1 << 31) + (1 << 25)) };
        ...
        void find_block1_stevens_00(UInt32[] block, UInt32[] IV)
        {
            ...
            while (true)
            {
                ...
                UInt32 tt0 = FF(Q[Qoff + 0], Q[Qoff - 1], Q[Qoff - 2]) + Q[Qoff - 3] + 0xd76aa478;
                UInt32 tt1 = Q[Qoff - 2] + 0xe8c7b756;
                UInt32 q1a = 0x02020801 | (Q[Qoff + 0] & 0x80000000);
        ...
        void find_block1_stevens_01(UInt32[] block, UInt32[] IV)
        {
            ...
            UInt32[] q9q10mask = new UInt32[1 << 5];
            for (UInt32 k = 0; k < q9q10mask.Length; ++k)
            ...
            while (true)
            {
                ...
                UInt32 q1a = 0x02000021 ^ (Q[Qoff + 0] & 0x80000020);
        ...
        void find_block1_stevens_10(UInt32[] block, UInt32[] IV)
        {
            ...
            UInt32[] q9q10mask = new UInt32[1 << 4];
            for (UInt32 k = 0; k < q9q10mask.Length; ++k)
            ...
            while (true)
            {
                ...
                UInt32 q1a = 0x02000941 ^ (Q[Qoff + 0] & 0x80000000);
        ...
        void find_block1_stevens_11(UInt32[] block, UInt32[] IV)
        {
            ...
            while (true)
            {
                ...
                UInt32 q1a = 0x02000861 ^ (Q[Qoff + 0] & 0x80000020);
        ...
        void find_block1_wang(UInt32[] block, UInt32[] IV)
        {
            ...
            UInt32[] q4mask = new UInt32[1 << 6];
            for (UInt32 k = 0; k < q4mask.Length; ++k)
                q4mask[k] = ((k << 13) ^ (k << 19)) & 0x01c0e000;

            UInt32[] q9mask = new UInt32[1 << 5], q10mask = new UInt32[1 << 5];
            for (UInt32 k = 0; k < q9mask.Length; ++k)
            {
            ...
            while (true)
            {
                ...
                UInt32 q1a = 0x04200040 | (Q[Qoff + 2] & 0xf01e1080);
