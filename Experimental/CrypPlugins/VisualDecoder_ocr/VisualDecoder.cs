...
using VisualDecoder_ocr.Decoders;
...
namespace VisualDecoder_ocr
{
    ...
    public class VisualDecoder : ICrypComponent
    {
        ...
        // decoder chain (we want the decoder to have an similuar architecture as the encoder)
        private readonly Dictionary<VisualDecoderSettings.DimCodeType, ImageDecoder> codeTypeHandler 
                                              = new Dictionary<VisualDecoderSettings.DimCodeType, ImageDecoder>();

        private OCRDecoder ORCDecoder;
        #endregion

        public VisualDecoder()
        {
            threadStart = ProcessInput;
            ORCDecoder = new OCRDecoder(this);

            codeTypeHandler.Add(VisualDecoderSettings.DimCodeType.AUTO, new ZXingDecoder(this));
