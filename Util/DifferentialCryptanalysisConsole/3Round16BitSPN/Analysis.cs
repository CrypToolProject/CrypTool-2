...
namespace _3Round16BitSPN
{
    class Analysis : IDifferentialCryptanalysis
    {
            ...
            */
            ...
            Parallel.For(0, loopBorder, i =>
            {
                ...
                foreach (var curPair in configuration.FilteredPairList)
                {
                    ...
                    for (int j = 0; j < _3Round16BitSPNConfiguration.SBOXNUM; j++)
                    {
        ...
        public int ReverseSBoxBlock(int data)
        {
            ...
            //use sbox
            zeroToThreeInt = _3Round16BitSPNConfiguration.SBOXREVERSE[zeroToThreeInt];
            fourToSevenInt = _3Round16BitSPNConfiguration.SBOXREVERSE[fourToSevenInt];
            eightToElevenInt = _3Round16BitSPNConfiguration.SBOXREVERSE[eightToElevenInt];
            twelveToFifteenInt = _3Round16BitSPNConfiguration.SBOXREVERSE[twelveToFifteenInt];
        ...
        public int ReversePBoxBlock(int data)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                pboxedArray[_3Round16BitSPNConfiguration.PBOXREVERSE[i]] = bitsOfBlock[i];
            }
        ...
        private List<Characteristic> FindAllCharacteristics(int round, List<SBoxCharacteristic> differentialsList, int outputDiff, Characteristic res)
        {
            ...
            if ((res.Probability < _3Round16BitSPNConfiguration.PROBABILITYBOUNDDIFFERENTIALSEARCH) && (res.Probability != -1))
                return null;
            ...
            bool[] activeSBoxes = new bool[_3Round16BitSPNConfiguration.SBOXNUM];

            //check active SBoxes
            int[] outputDiffs = new int[_3Round16BitSPNConfiguration.SBOXNUM];
            for (int i = 0; i < _3Round16BitSPNConfiguration.SBOXNUM; i++)
            {
            ...
            List<SBoxCharacteristic>[] arrayOfDifferentialLists = new List<SBoxCharacteristic>[_3Round16BitSPNConfiguration.SBOXNUM];
            int comb = 1;
            for (int b = 0; b < _3Round16BitSPNConfiguration.SBOXNUM; b++)
            {
                if (activeSBoxes[b])
                {
            ...
            {
                SBoxCharacteristic[] curDiffSBoxes = new SBoxCharacteristic[_3Round16BitSPNConfiguration.SBOXNUM];
                ...
                //check null values
                for (int z = 0; z < _3Round16BitSPNConfiguration.SBOXNUM; z++)
                {
                ...
                int inputDiff = 0;
                for (int i = _3Round16BitSPNConfiguration.SBOXNUM - 1; i >= 0; i--)
                {
                    inputDiff = inputDiff ^ curDiffSBoxes[i].InputDifferential;
                    if ((i - 1) >= 0)
                    {
                        inputDiff = inputDiff << _3Round16BitSPNConfiguration.BITWIDTHCIPHERFOUR;
                    }
                }
            ...
            Characteristic best = new _3Round16BitSPNCharacteristic();
        ...
        public List<Characteristic> FindCharacteristics(int expectedDiff, int round, List<SBoxCharacteristic> differentialNumList)
        {
            ...
            //Starting object
            Characteristic inputObj = new _3Round16BitSPNCharacteristic();
        ...
        public Characteristic FindBestCharacteristic(int round, List<SBoxCharacteristic> differentialsList, int outputDiff, Characteristic res)
        {
            //break if probability is not good enough
            // ReSharper disable once CompareOfFloatsByEqualityOperator
            if ((res.Probability < _3Round16BitSPNConfiguration.PROBABILITYBOUNDBESTCHARACTERISTICSEARCH) && (res.Probability != -1))
                return null;
            ...
            DifferentialKeyRecoveryAttack attack, IEncryption encryption, int expectedDifferential)
        {
            // cast to use the object
             _3Round16BitSPNKeyRecoveryAttack cipherFourAttack = attack as _3Round16BitSPNKeyRecoveryAttack;
            ...
            List<int>[] arrayOfPossibleDifferentialsForSBoxes = new List<int>[_3Round16BitSPNConfiguration.SBOXNUM];
            int[] arrayOfExpectedInputDifferentialsForSBoxes = new int[_3Round16BitSPNConfiguration.SBOXNUM];
            ...
            foreach (var curPair in roundConfig.UnfilteredPairList)
            {
                ...
                */
                ...
                int[] diffOfCipherTextSBoxes = new int[_3Round16BitSPNConfiguration.SBOXNUM];
                bool[] conditionsOfSBoxes = new bool[_3Round16BitSPNConfiguration.SBOXNUM];
                for (int i = 0; i < _3Round16BitSPNConfiguration.SBOXNUM; i++)
                {
        ...
        public int PartialDecrypt(DifferentialKeyRecoveryAttack attack, int block)
        {
            ...
            {
                result = DecryptSingleRound(result, cipherFourAttack.subkey3, false, true);
            }

            if (cipherFourAttack != null && cipherFourAttack.recoveredSubkey2)
            {
                result = DecryptSingleRound(result, cipherFourAttack.subkey2, true, false);
            }
        ...
        public DifferentialAttackRoundConfiguration GenerateConfigurationAttack(int round, bool[] sBoxesToAttack,
            AbortingPolicy abortingPolicy, SearchPolicy searchPolicy, List<SBoxCharacteristic> diffListOfSBox, DifferentialKeyRecoveryAttack attack,
                ...
                case SearchPolicy.FirstBestCharacteristicDepthSearch:
                    {
                        ...
                        {
                            bool[] conditionArray = new bool[_3Round16BitSPNConfiguration.SBOXNUM];
        ...
        public int CalculateLoopBorder(bool[] activeSBoxes)
        {
            ...
            border = (int)Math.Pow(2, (border * _3Round16BitSPNConfiguration.BITWIDTHCIPHERFOUR));
            return border;
        }
        ...
        public int GenerateValue(bool[] activeSBoxes, int data)
        {
            ...
            for (int i = 0; i < _3Round16BitSPNConfiguration.SBOXNUM; i++)
            {
                if (activeSBoxes[i])
                {
                    for (int j = 0; j < _3Round16BitSPNConfiguration.BITWIDTHCIPHERFOUR; j++, currentActiveBitPosition++)
                    {
                        result[(i * _3Round16BitSPNConfiguration.BITWIDTHCIPHERFOUR) + j] = bitsOfValue[currentActiveBitPosition];
                    }
                }
            }
